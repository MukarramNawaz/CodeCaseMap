...
import { EllipsisHorizontalIcon, PencilIcon, TrashIcon, CheckIcon } from '@heroicons/react/24/solid';
...
const ChatItem = ({ chat, onRename, currentChatId, onDelete, onChatClick }) => {
  ...
  return (
    <div className={`py-2 px-3  rounded-lg relative group ${currentChatId === chat.id ? "bg-gray-300" :" hover:bg-gray-100"}`}>
      {!isEditing ? (
        <div className="flex justify-between items-center">
          <div className="flex gap-1 items-center cursor-pointer w-full" onClick={ () => onChatClick(chat)}>
          <MessageCircleMore className="h-5 w-5 " />
          <span>{chat.name.slice(0,20)} {chat.name.length >20 && "..."}</span>
          </div>
          ...
          <Menu as="div" className="relative">
            ...
            <MenuItems className="absolute right-0 mt-1 w-36 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
              <MenuItem>
                {({ active }) => (
                  <button
                    className={`${
        ...
        <div className="fixed inset-0  flex items-center justify-center z-50 animate-fade-in">
          <div className="bg-white w-11/12 md:max-w-md md:w-full p-6 rounded-lg shadow-lg ">
            ...
            <div className="mt-6 flex justify-center space-x-4">
              ...
              <button className="bg-red-600 text-white px-10 py-2 rounded-3xl hover:bg-red-800 focus:outline-none"
